...
const DroppableCell = ({ id, card, flashClass, element, isLevitating = false, index = 0 }) => {
  ...
  const col = index % 5;
  const dx = `${(2 - col) * 110}%`; // Center is (2,2)
  const dy = `${(2 - row) * 110}%`;
...
export default function GameBoard5x5({ matchConfig = { basicRules: ['basic'], specialRule: null, infectionRule: null }, onReset }) {
  ...
  const [boardElements, setBoardElements] = useState(Array(25).fill(null));
  ...
  const initGame = (customPlayerHand = null, customOpponentHand = null) => {
    ...
    if (customPlayerHand && customOpponentHand) {
      ...
      const generateTestHand = (owner) => Array.from({ length: 13 }, () => 
        generateCard(Math.random() < 0.2 ? 'LEGENDARY' : Math.random() < 0.5 ? 'EPIC' : 'RARE', owner)
    ...
    const newElements = Array(25).fill(null);
    if (activeRules.includes('elemental')) {
      const elTypes = ['💧', '🔥', '🌿', '🪨', '⚡', '✨', '🌑'];
      let numToPlace = Math.floor(Math.random() * 5) + 4; // 4-8 elements for 5x5
      while (numToPlace > 0) {
  ...
  const checkWin = (b) => {
    ...
    const emptyCount = b.filter(c => c === null).length;
  ...
  const executeMove = (card, position) => {
    const { newBoard, capturedBy, captureSequence } = placeCardOnBoard(board, card, position, 5, activeRules, boardElements);
    ...
    const finalize = (finalBoard) => {
      ...
      if (result) {
        if (result === 'draw' && activeRules.includes('sudden_death')) {
          ...
          // Deal 1 extra random card to each to make it 13 again (since 12 were played)
          newPHand.push(generateCard('RARE', 'player'));
          newOHand.push(generateCard('RARE', 'opponent'));
  ...
  useEffect(() => {
    ...
    const t = setTimeout(() => {
      const move = calculateBestMove(board, opponentHand, 5, activeRules, boardElements);
